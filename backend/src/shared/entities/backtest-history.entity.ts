...
@Entity("backtest_history", { 
  comment: '回测历史表：存储用户执行回测的历史记录，包括回测配置、结果、状态等信息' 
})
export class BacktestHistory {
   ...
   * 存储买卖条件、止损止盈等交易规则
   */
  @Column({ name: "tradingRules", type: "json", nullable: true, comment: '交易规则配置（JSON格式）' })
  tradingRules?: any;
   ...
   * 模拟实际交易中的价格滑点
   */
  @Column({ name: "slippage", type: "decimal", precision: 10, scale: 6, default: 0.001, comment: '滑点设置' })
  slippage?: number;
   ...
   * 设置单笔交易的最小金额限制
   */
  @Column({ name: "minTradeAmount", type: "decimal", precision: 15, scale: 2, default: 1000, comment: '最小交易金额' })
